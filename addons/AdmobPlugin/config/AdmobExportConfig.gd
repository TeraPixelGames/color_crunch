	...
	if __load_result == Error.OK:
		...
		if __config_file.has_section(CONFIG_FILE_SECTION_MEDIATION):
			if __config_file.has_section_key(CONFIG_FILE_SECTION_MEDIATION, CONFIG_FILE_KEY_ENABLED_NETWORKS):
				var __network_array_raw: Variant = __config_file.get_value(CONFIG_FILE_SECTION_MEDIATION, CONFIG_FILE_KEY_ENABLED_NETWORKS, [])
				var __network_array: Array = __network_array_raw if __network_array_raw is Array else []

				for __network in __network_array:
					var __network_tag: String = str(__network)
					if MediationNetwork.is_valid_tag(__network_tag):
						enabled_mediation_networks.append(MediationNetwork.get_by_tag(__network_tag))
					else:
						Admob.log_error("Invalid network tag '%s' in file %s!" % [__network_tag, __config_file_path])
			else:
